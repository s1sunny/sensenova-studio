...
**文生图 / 图像编辑 / 任务队列 / 产物管理 / 请求日志** 的图形化界面，无需编写调用代码。
...
- **图像编辑**：参考图支持本地上传 / URL 引入 / 从产物画廊选图，自动转 base64，多参考图组合
- **产物管理**：生成图片落盘 `data/artifacts/`（图片 + meta.json），网格画廊、Prompt 搜索、一键复制、删除同步清理磁盘
...
| `GET` | `/api/artifacts` | 产物列表（分页 / 搜索 / 类型筛选） |
| `GET` | `/api/artifacts/:id/image` | 产物图片 |
| `DELETE` | `/api/artifacts` | 批量删除产物 `{ ids: [] }` |
...
│   ├── artifacts.js       # 产物落盘与索引（data/artifacts/）
...
- **生成的图存在哪里**：`data/artifacts/<产物ID>/`（`image.png` + `meta.json`）。
